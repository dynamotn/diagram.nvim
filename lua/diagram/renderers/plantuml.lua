...
-- fs cache
local cache_dir =
  vim.fn.resolve(vim.fn.stdpath('cache') .. '/diagram-cache/plantuml')
...
M.render = function(source, options)
  ...
  if not vim.fn.executable('plantuml') then
    error('diagram/plantuml: plantuml not found in PATH')
  end
  ...
  local job_id = vim.fn.jobstart(
    command .. ' > ' .. vim.fn.shellescape(path .. '.new.png'), -- HACK: write to .new.png to prevent rendering a incomplete file
    {
      ...
      on_stderr = function(job_id, data, event)
        ...
        vim.notify(
          'diagram/plantuml: plantuml failed to render diagram.\nError: '
            .. error_msg,
