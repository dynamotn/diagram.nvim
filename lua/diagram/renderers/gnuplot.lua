...
-- fs cache
local tmpdir =
  vim.fn.resolve(vim.fn.stdpath('cache') .. '/diagram-cache/gnuplot')
...
M.render = function(source, options)
  ...
  if options.font then
    table.insert(
      script,
      string.format("set terminal pngcairo font '%s'", options.font)
    )
  end
  ...
  local job_id = vim.fn.jobstart(command, {
    ...
    on_stderr = function(job_id, data, event)
      ...
      vim.notify(
        'diagram/gnuplot: gnuplot failed to render diagram' .. error_msg,
